...
test("text-list scope enables only explicit message and scalar templates", () => {
  const value = template([{
    ...
    body: [
      { id: "send", type: "send", terminal: worker, message: { parts: [{ kind: "template", template: "send {{index}}/{{key}}/{{value}}" }, { kind: "text", text: "literal {{value}}" }] }, delivery: "direct", ending: "cr" },
      ...
      { id: "input", type: "input", terminal: worker, prompt: { kind: "template", template: "input {{key}}" }, allowEmpty: true, delivery: "direct", ending: "none" },
      { id: "continue_wait", type: "wait", mode: "user-continue", prompt: { kind: "template", template: "continue {{index}}" } },
      {
        id: "parallel",
        type: "parallel",
        lanes: [
          { id: "lane_worker", label: "Worker", terminal: worker, body: [{ id: "lane_send_worker", type: "send", terminal: worker, message: { parts: [{ kind: "template", template: "lane {{index}}/{{key}}/{{value}}" }] }, delivery: "direct", ending: "cr" }, { id: "lane_output_worker", type: "output", source: { kind: "none" } }] },
...
test("count and if descendants inherit outer text-list scope", () => {
  const value = template([{
    ...
    body: [{
      ...
      body: [{ id: "send_inner", type: "send", terminal: worker, message: { parts: [{ kind: "template", template: "{{value}}" }] }, delivery: "direct", ending: "cr" }],
    }],
  }])
...
test("templates outside text-list scope and malformed list/templates fail loudly", () => {
  const outside = template([{ id: "send", type: "send", terminal: worker, message: { parts: [{ kind: "template", template: "{{value}}" }] }, delivery: "direct", ending: "cr" }])
  expect(issueText(outside)).toContain("template requires an enclosing text-list for")

  const literal = template([{ id: "send", type: "send", terminal: worker, message: { parts: [{ kind: "text", text: "{{value}}" }] }, delivery: "direct", ending: "cr" }])
  expect(validateFlowV2Template(literal, { indexMap }).ok).toBe(true)

  const malformed = template([{ id: "loop", type: "for", range: { kind: "text-list", items: [] }, body: [{ id: "send", type: "send", terminal: worker, message: { parts: [{ kind: "template", template: "{{foo}}" }] }, delivery: "direct", ending: "cr" }] }])

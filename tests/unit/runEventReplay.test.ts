...
test('replay reports missing artifact refs as recoverable error', () => {
  ...
  try {
    writeEvents(root, 'local', 'run_missing_artifact', [
      ...
      event('run_missing_artifact', 2, 'evt_artifact', 'terminal_text_sent', { terminalId: 'term_worker', delivery: "direct", ending: 'none', content: { artifactRef: 'artifacts/missing.txt', chars: 5 }, write: { artifactRef: 'artifacts/missing.txt', chars: 5 } }, 'send_review'),
    ])

...
function template(): MacroTemplate {
  ...
  return {
    ...
    updatedAt: now,
    body: [
      { id: "send_1", type: "send", terminal: { kind: "index", value: 1 }, message: { parts: [{ kind: "text", text: "one" }] }, delivery: "direct", ending: "cr" },
      {
        id: "if_ready",
        type: "if",
...
test("inside control insertion creates missing optional body only during mutation", () => {
  const t = template()
  t.body.push({ id: "done", type: "finish", reason: "done" })
  const result = insertNodeAtAnchor(t, { kind: "inside", parentPath: [], index: 3, slot: "control", anchorNodeId: "done" }, { id: "send_before_finish", type: "send", terminal: { kind: "index", value: 1 }, message: { parts: [{ kind: "text", text: "before finish" }] }, delivery: "direct", ending: "cr" })
...
test("control terminal bodies accept actions but reject flow nodes", () => {
  ...
  expect(insertNodeAtAnchor(t, { kind: "inside", parentPath: [], index: 3, slot: "control" }, { id: "send_before_finish", type: "send", terminal: { kind: "index", value: 1 }, message: { parts: [{ kind: "text", text: "before finish" }] }, delivery: "direct", ending: "cr" }).ok).toBe(true)

...
test('active run visibly locks Macro authoring and highlights the frozen current stage until stop', async ({ page, request }) => {
  ...
  await expect(page.getByTestId('macro-editor-lock-notice')).toHaveAttribute('data-lock-reason', 'macro_run_active')
  ...
  await expect(page.getByTestId('macro-current-stage')).toContainText('wait · wait')
  await expect(page.getByTestId('macro-current-stage')).toHaveCSS('border-top-width', '2px')
  await expect(waitNode).toHaveCSS('outline-style', 'solid')

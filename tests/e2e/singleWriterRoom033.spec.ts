...
test('same Room has one writer while observer keeps local controls and can explicitly take over', async ({ browser, request }) => {
  ...
  await expect(second.getByRole('button', { name: 'New shell' })).toHaveAttribute('aria-disabled', 'true')
  await expect(second.getByRole('button', { name: 'New text' })).toHaveAttribute('aria-disabled', 'true')
  await expect(second.getByTestId('terminal-tab-close')).toHaveAttribute('aria-disabled', 'true')
  ...
  await second.getByRole('button', { name: 'New shell' }).click({ force: true })
  await expect(second.getByTestId('notice-item')).toContainText('This device is read-only')
  await second.getByTestId('notice-item').hover()
  await second.waitForTimeout(3_100)
  await expect(second.getByTestId('notice-item')).toBeVisible()
  await second.mouse.click(4, 4)
  await expect(second.getByTestId('notice-item')).toHaveCount(0)

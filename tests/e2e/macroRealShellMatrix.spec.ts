...
test('real shell terminal target refs support index id and alias', async ({ page, request }) => {
  ...
  await importTemplate(request, configId, baseTemplate(configId, 'target_refs', 'Target Ref Smoke', [
    { id: 'send_index', type: 'send', terminal: { kind: 'index', value: 1 }, message: { parts: [{ kind: 'text', text: printf('SD_TARGET_INDEX_010') }] }, ending: "cr" },
    { id: 'send_id', type: 'send', terminal: { kind: 'id', value: first.terminalId }, message: { parts: [{ kind: 'text', text: printf('SD_TARGET_ID_010') }] }, ending: "cr" },
    { id: 'send_alias', type: 'send', terminal: { kind: 'alias', value: 'shell_2' }, message: { parts: [{ kind: 'text', text: printf('SD_TARGET_ALIAS_010') }] }, ending: "cr" },
...
test('real shell text_match if handles simple true branch', async ({ page, request }) => {
  ...
  await importTemplate(request, configId, baseTemplate(configId, 'text_match_branch', 'Text Match Branch', [
    { id: 'send_branch', type: 'send', terminal: target, message: { parts: [{ kind: 'text', text: printf('SD_BRANCH_TRUE_010') }] }, ending: "cr" },
  ...
  await importTemplate(request, configId, baseTemplate(configId, "real_parallel_lane_output", "Real Parallel Lane Output", [
    {
      ...
      lanes: [
        { id: "docs", label: "Docs", terminal: { kind: "alias", value: "shell_1" }, body: [
          { id: "send_docs", type: "send", terminal: { kind: "alias", value: "shell_1" }, message: { parts: [{ kind: "text", text: printf("SD_PARALLEL_DOCS_010") }] }, ending: "cr" },
        ...
        { id: "tests", label: "Tests", terminal: { kind: "alias", value: "shell_2" }, body: [
          { id: "send_tests", type: "send", terminal: { kind: "alias", value: "shell_2" }, message: { parts: [{ kind: "text", text: printf("SD_PARALLEL_TESTS_010") }] }, ending: "cr" },

...
* `src/lib/protocol.ts`、`src/lib/macro/runnerTypes.ts`：破坏性收口bounded `runner_snapshot`、live `runner_delta`、`content_record_changed`、`runtimeRevision`、`runningMacro`和server-owned runtime input；旧flattened snapshot、每次transition全量重发events与旧input submit body不再接受。
* `server/macroRunnerService.ts`、`server/httpServer.ts`：Room内存成为runner/runtime input唯一真值；连接/重连发送一次bounded完整snapshot，状态变化只publish最新state与新增event delta，快速同步transition最多25ms合并；draft update/submit执行controller、Room generation、run、invocation和revision复核。
...
同日长run与Room projection复审收口：WebSocket只在连接/repair发送最近durable tail，正常状态变化发送monotonic `runner_delta`；client按absolute eventSeq合并，gap只触发一次完整repair。Room snapshot与index map使用独立channel watermark并共同服从已观察到的roomRevision；stale terminal event只有在terminal stamp被接受时才能派生readiness。`runner_input_requested` evidence只保留invocationId、inputRevision、hasDefaultText和promptChars，不持久化prompt/default/draft/value正文。

...
实现上，`createLibrarySession`继续是kind/search/items/selection/draft/lease及全部Svelte rune state的唯一owner、commit gateway和public assembly point，既有`LibraryNavigationCoordinator`继续唯一冻结operation/controller/kind/selection/draft/revision/lease identity。`LibraryListCoordinator`只持有180ms search timer与list request generation，读取live kind/query并把typed result交回factory；`LibraryEditOrchestrator`只编排kind/selection/New、lease、persist/delete、Cancel/Discard与Macro Load request phase，所有结果仍由factory复核current identity后安装。既有Library-specific `LibraryMutationWorkflow`继续只处理client与mutation transaction，`LibraryRemoteSyncCoordinator`继续只持有invalidation queue、retry timer、read/connection generation与serialized drain。四个helper都不缓存第二份item/draft/lease state，也不与Macro共享generic content session。

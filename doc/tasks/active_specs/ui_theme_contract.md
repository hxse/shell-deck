...
Structure Gate消费由`20260722A.002`父revision生成的32-file稳定per-file fixture：父基线835个non-presentation entry，current只允许App Theme field新增label/span/select/option四个element，以及`20260723A`明确授权的两条attribute-level interaction delta——`MacroEditorShell`既有notice的conditional role/tabindex/click/keydown与`MacroPanel`到canonical `beginEdit()`的prop wiring；element总数仍精确为839。fixture为这两个文件补齐父版本per-entry digest，测试断言精确removed digest与added entry，不靠替换current aggregate hash放行。fingerprint只排除class/style presentation；terminal identity、palette、viewport、cursor与selection测试状态不得写入production `data-terminal-*` attribute，而由测试显式dispatch opt-in request event同步读取。

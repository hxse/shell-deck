...
revision conflict、invalid patch、server/client合并后hash mismatch都进入single-flight full Text snapshot repair。full snapshot同样带完整正文hash；repair后再次不一致必须fail loudly并停止自动循环。repair budget绑定terminalId/launchId，reset后的新launch不继承旧launch的失败次数，旧launch迟到message也不能消耗新budget。每次accepted full Text projection分配进程内不可复用的monotonic token；异步hash返回后必须重新核对Room generation、token、backend、launchId和textRevision，reset/reconnect期间的旧结果不得安装或消耗新budget。repair期间的latest local edit在authoritative正文上重新计算并提交，不能静默丢失。blur、terminal切换/关闭、拖拽切换与Start等会改变读取边界的in-app action先flush；controller loss/transport interruption明确取消等待并恢复server truth，browser unload在pending时保留native warning。Text与Macro multiline gutter只渲染viewport visible lines加固定overscan，不按总行数创建DOM。
...
control实现保持单向组合：内部`roomControlCoordinator`是`TerminalRoomManager`消费的唯一facade；`roomClientPresenceCoordinator`只负责client registration、disconnect、ping/pong与heartbeat sweep，`roomControllerLeaseCoordinator`唯一转换`room.controller/controlEpoch`并负责personalized view、takeover/release与controlled ticket。二者直接操作manager创建的同一组Room/client map reference，不建立cache、shadow registry或第二份owner state；其他production consumer不得直接拼装internal coordinator。Room WebSocket upgrade冻结的generation必须进入同一同步registration临界区，并在client ID、registry、message及controller mutation前核对；mismatch零副作用失败，registration后续send失败则删除本次client并仅按exact provisional owner identity恢复fresh control epoch，因此旧socket不能成为新generation controller或留下ghost client。takeover在old-owner cleanup await后仍复核Room lifecycle与new owner context，published-operation仍只在publish前鉴权，不能因模块拆分新增post-commit false failure。

browser Workspace同样保持单向组合：`createRoomWorkspaceState`唯一声明`active/connected/generation/control/terminal/runner/content/selection` runes并保留全部terminal UI commands；message coordinator无socket/effect/cache，只经live commit ports维持generation filter、revision gate、runner repair与最近200项content change sequence；reconnect coordinator只拥有750ms single retry、5秒reload reclaim与session intent。成功Room probe确认关闭时的exact Room/generation：Room消失进入Home、仍存在则retry；network exception继续retry，HTTP非成功或body `ok`为false继续直接返回。拆分不新增close/acquire continuation token，普通observer不会被自动takeover。

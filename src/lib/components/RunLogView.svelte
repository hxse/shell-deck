<script lang="ts">
  ...
  async function appendDemoEvents() {
    ...
    try {
      ...
      await api.appendEvent(runId, { kind: 'terminal_text_sent', stepId: 'send_review', summary: 'Sent review prompt', data: { terminalRef: { kind: 'alias', value: 'reviewer' }, ending: 'cr', content: { artifactRef: sendArtifact.artifact.artifactRef, chars: 16 }, write: { artifactRef: sendWriteArtifact.artifact.artifactRef, chars: 17 } } })

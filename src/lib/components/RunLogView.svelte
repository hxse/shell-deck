<script lang="ts">
  ...
  async function appendDemoEvents() {
    ...
    try {
      ...
      const sendWriteArtifact = await api.writeArtifact(runId, { prefix: 'send-write', content: 'review docs only\r', extension: 'txt', stepId: 'send_review' })
      await api.appendEvent(runId, { kind: 'terminal_text_sent', stepId: 'send_review', summary: 'Sent review prompt', data: { terminalRef: { kind: 'alias', value: 'reviewer' }, delivery: 'direct', resolvedDelivery: 'direct', ending: 'cr', content: { artifactRef: sendArtifact.artifact.artifactRef, chars: 16 }, write: { artifactRef: sendWriteArtifact.artifact.artifactRef, chars: 17 } } })

...
<section class="macro-run-status macro-run-status-dock" data-testid="macro-run-status">
  ...
  <div class="macro-run-controls" data-testid="macro-run-controls">
    <button type="button" class="macro-prepare-terminals" data-testid="macro-prepare-terminals" title={prepareDisabled ? prepareDisabledReason : 'Prepare current draft terminal layout'} disabled={prepareDisabled || preparing} onclick={onPrepare}>{preparing ? 'Preparing…' : 'Prepare terminals'}</button>
    <button type="button" data-testid="macro-control-start" title={startDisabled ? startDisabledReason : 'Start saved macro revision'} disabled={startDisabled} onclick={() => onMacroControl('start')}>Start</button>
    <button type="button" data-testid="macro-control-pause-resume" title={pauseResumeDisabled() ? 'No live run to pause or resume' : pauseResumeAction() === 'resume' ? 'Resume run' : 'Pause run'} disabled={pauseResumeDisabled()} onclick={() => onMacroControl(pauseResumeAction())}>{pauseResumeAction() === 'resume' ? 'Resume' : 'Pause'}</button>
    <button type="button" data-testid="macro-control-stop" title="Stop active run" disabled={!runner || !['running', 'paused', 'waiting_input', 'stopping'].includes(runner.status)} onclick={() => onMacroControl('stop')}>Stop</button>
  </div>
  ...
  {#if runner?.inputPrompt}
    <div class="runner-input-line" data-testid="macro-run-input">
      <label>{runner.inputPrompt}<LineNumberedTextarea testId="macro-run-input-text" value={runnerInput} maxRows={4} ariaLabel="Runtime input line" onInput={onRunnerInputChange} /></label>

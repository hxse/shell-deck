...
<section class="macro-run-status macro-run-status-dock grid min-w-0 grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-1 rounded-field bg-base-200 px-1.5 py-1 text-[11px] shadow-sm" data-testid="macro-run-status" data-run-status={runner?.status ?? 'idle'}>
  <div class="macro-run-summary grid min-w-0 gap-1">
    ...
    {#if runner && isActiveMacroRunnerStatus(runner.status)}
      <div class="macro-current-stage alert alert-warning flex min-w-0 items-center gap-2 px-2 py-1 font-bold" data-testid="macro-current-stage" data-current-node-id={runner.currentNodeId ?? undefined}>
        <span class="macro-current-stage-label shrink-0 pr-2 text-[10px] tracking-wide uppercase">Current stage</span>
  ...
  <details class="macro-debug-refresh relative text-[11px]" data-testid="macro-debug-refresh"><summary class="whitespace-nowrap" data-testid="macro-debug-toggle">Debug</summary><button class="btn btn-xs btn-ghost absolute top-[calc(100%+4px)] right-0 z-[4] shadow-lg" type="button" data-testid="macro-run-refresh" onclick={onRefreshRunner}>Refresh</button></details>
  {#if runner?.runtimeInput}
    <div class="runner-input-line alert alert-error col-span-full grid grid-cols-[minmax(0,1fr)_auto] items-end gap-2 p-2" data-testid="macro-run-input">
      ...
      <button class="btn btn-xs btn-primary !pointer-events-auto aria-disabled:cursor-not-allowed" type="button" data-testid="macro-run-input-submit" aria-disabled={runtimeInputDisabled || runnerInputSyncing} onclick={onSubmitRunnerInput}>{runnerInputSyncing ? 'Syncing…' : 'Send'}</button>
    </div>
  {/if}
</section>

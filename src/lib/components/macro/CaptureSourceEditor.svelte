...
{#if allowedKinds.length > 1}
  {#if variant === 'root'}
    <label>Capture kind<select data-testid="capture-step-kind" value={capture.kind} onchange={(event) => onKindChange(event.currentTarget.value as CaptureSourceConfig['kind'])}>{#each allowedKinds as kind}<option value={kind}>{kind}</option>{/each}</select></label>
  {:else}
    <label>Capture kind<select data-testid="parallel-capture-kind" value={capture.kind} onchange={(event) => onKindChange(event.currentTarget.value as CaptureSourceConfig['kind'])}>{#each allowedKinds as kind}<option value={kind}>{kind}</option>{/each}</select></label>
  {/if}
...
{:else if capture.kind === 'terminal-buffer'}
  {#if variant === 'root'}
    ...
    <label>Max chars<input data-testid="capture-max-chars" type="number" value={capture.maxChars} oninput={(event) => changeCapture({ ...capture, maxChars: Number(event.currentTarget.value) })} /></label>
  {:else}
    <div class="macro-row">
      <label>Mode<select data-testid="parallel-capture-mode" value={capture.mode} onchange={(event) => changeCapture({ ...capture, mode: event.currentTarget.value as 'scrollback-tail' | 'raw-stream-tail' })}><option value="scrollback-tail">screen text</option><option value="raw-stream-tail">raw stream tail</option></select></label>
      <label>Max chars<input data-testid="parallel-capture-max-chars" type="number" value={capture.maxChars} oninput={(event) => changeCapture({ ...capture, maxChars: Number(event.currentTarget.value) })} /></label>
    </div>
  {/if}
{:else if capture.kind === 'agent-event'}
  {#if variant === 'root'}
    <label>Agent<select data-testid="capture-agent-kind" value={capture.agent.kind} onchange={() => changeCapture({ ...capture, agent: { kind: 'codex' } })}><option value="codex">codex</option></select></label>
  ...
  {:else}
    <div class="macro-row"><label>Agent<select data-testid="parallel-capture-agent-kind" value={capture.agent.kind} onchange={(event) => changeCapture({ ...capture, agent: { kind: event.currentTarget.value as 'codex' } })}><option value="codex">codex</option></select></label><label>Mode<select data-testid="parallel-capture-agent-mode" value={capture.captureMode ?? 'result_only'} onchange={(event) => changeCapture({ ...capture, captureMode: event.currentTarget.value as 'result_only' | 'prompt_only' | 'prompt_and_result' })}><option value="result_only">result only</option><option value="prompt_only">prompt only</option><option value="prompt_and_result">prompt + result</option></select></label></div>
  {/if}
  {#if variant === 'root'}
    <div class="agent-wait-limit">
      ...
      {#if capture.waitLimit.kind === 'timeout'}
        <label class="agent-timeout-duration"><span>Timeout ms</span><input data-testid="capture-agent-timeout-ms" type="number" min="1" step="1" value={capture.waitLimit.timeoutMs} oninput={(event) => { if (capture.kind === 'agent-event' && capture.waitLimit.kind === 'timeout') changeCapture({ ...capture, waitLimit: { kind: 'timeout', timeoutMs: Number(event.currentTarget.value) } }) }} /></label>
      {:else}
  ...
  {:else}
    <div class="agent-wait-limit">
      ...
      {#if capture.waitLimit.kind === 'timeout'}
        <label class="agent-timeout-duration"><span>Timeout ms</span><input data-testid="parallel-capture-agent-timeout-ms" type="number" min="1" step="1" value={capture.waitLimit.timeoutMs} oninput={(event) => { if (capture.kind === 'agent-event' && capture.waitLimit.kind === 'timeout') changeCapture({ ...capture, waitLimit: { kind: 'timeout', timeoutMs: Number(event.currentTarget.value) } }) }} /></label>
      {:else}

  ...
  <article class="step-card parallel-lane-action" data-testid="parallel-lane-action">
    ...
    {:else if item.type === "capture-source"}
      ...
      {:else if item.capture.kind === "agent-event"}
        <div class="macro-row"><label>Agent<select value={item.capture.agent.kind} onchange={(event) => updateLaneAction(lane.id, item.id, (action) => { if (action.type === "capture-source" && action.capture.kind === "agent-event") action.capture.agent = { kind: event.currentTarget.value as "codex" } })}><option value="codex">codex</option></select></label><label>Field<select value={item.capture.field} onchange={(event) => updateLaneAction(lane.id, item.id, (action) => { if (action.type === "capture-source" && action.capture.kind === "agent-event") action.capture.field = event.currentTarget.value as "last_assistant_message" })}><option value="last_assistant_message">last_assistant_message</option></select></label></div>
      {:else}

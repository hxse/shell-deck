import type { FlowV2IfBranch, FlowV2Node, MacroDefinitionV3 } from './macroDefinitionTypes'
...
export function resolveBodyPath(template: MacroDefinitionV3, path: BodyPath): FlowV2Node[] | undefined {
...
export function insertNodeAtAnchor(template: MacroDefinitionV3, anchor: InsertionAnchor, node: FlowV2Node): CommandResult {
...
export function removeNodeAtPosition(template: MacroDefinitionV3, position: NodePosition): CommandResult {
...
export function moveNodeAtPosition(template: MacroDefinitionV3, position: NodePosition, offset: -1 | 1): CommandResult {
  const body = resolveBodyPath(template, position.bodyPath)
  if (!body) return { ok: false, reason: "body_not_found" }
...
export function findNodePosition(template: MacroDefinitionV3, nodeId: string): NodePosition | undefined {
  return findNodePositionInBody(template.body, nodeId, [])
}

export function canMoveNodeToAnchor(template: MacroDefinitionV3, nodeId: string, anchor: InsertionAnchor): boolean {
...
export function isInsertionAnchorValid(template: MacroDefinitionV3, anchor: InsertionAnchor): boolean {
  return resolveInsertionTarget(template, anchor, false) !== undefined
}

export function moveNodeToAnchor(template: MacroDefinitionV3, nodeId: string, anchor: InsertionAnchor): CommandResult {
  const sourcePosition = findNodePosition(template, nodeId)
...
export function insertElifBranchAfter(template: MacroDefinitionV3, position: NodePosition, afterBranchIndex: number, branch: FlowV2IfBranch): CommandResult {
  const body = resolveBodyPath(template, position.bodyPath)
  if (!body) return { ok: false, reason: "body_not_found" }
...
export function removeIfBranchAt(template: MacroDefinitionV3, position: NodePosition, branchIndex: number): CommandResult {
  const body = resolveBodyPath(template, position.bodyPath)
  if (!body) return { ok: false, reason: "body_not_found" }
  const node = body[position.index]
  if (!node) return { ok: false, reason: "node_not_found" }
...
export function ensureElseForIfNode(template: MacroDefinitionV3, position: NodePosition): CommandResult {
  const body = resolveBodyPath(template, position.bodyPath)
  if (!body) return { ok: false, reason: "body_not_found" }
  const node = body[position.index]
  if (!node) return { ok: false, reason: "node_not_found" }
...
export function removeElseFromIfNode(template: MacroDefinitionV3, position: NodePosition): CommandResult {
  const body = resolveBodyPath(template, position.bodyPath)
  if (!body) return { ok: false, reason: "body_not_found" }
  const node = body[position.index]
  if (!node) return { ok: false, reason: "node_not_found" }
...
function resolveInsertionTarget(template: MacroDefinitionV3, anchor: InsertionAnchor, createMissingBody: boolean): InsertionTarget | undefined {
  if (anchor.kind === "before" || anchor.kind === "after") {
    const body = resolveBodyPath(template, anchor.parentPath)
...
function cloneTemplateForCommandProbe(template: MacroDefinitionV3): MacroDefinitionV3 {
...
function findNodeById(template: MacroDefinitionV3, nodeId: string): FlowV2Node | undefined {

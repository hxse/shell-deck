...
{:else if workspace}
  <main class="room-shell">
    <header class="topbar compact-topbar">
      <div class="brand-line"><h1>shell-deck</h1><p data-testid="room-identity">{roomId} · {workspace.connected ? 'connected' : 'disconnected'}</p></div>
      <div class="actions">
        ...
        <button type="button" class="terminal-create-button" data-testid="terminal-create-real" onclick={workspace.createShell} aria-disabled={!workspace.canMutateShared || workspace.terminalStructureLocked}>New shell</button>
        <button type="button" class="terminal-create-button" data-testid="terminal-create-text" onclick={workspace.createText} aria-disabled={!workspace.canMutateShared || workspace.terminalStructureLocked}>New text</button>
    ...
    {#if settingsOpen}
      ...
      <section class="settings-popover" data-testid="settings-popover">
        ...
        <button type="button" class="settings-control-button" data-testid="notification-success-sound-test" onclick={() => { void workspace.playNotificationSound('success', 'success') }}>Play success sound</button>
      </section>
    {/if}
